    ...
    if (!!this.controller) {
      ...
      this.controller.valueChanges.subscribe((password: string) => {
        ...
        if(password.length>12 && mayusculas && minusculas && numeros && simbolos){
         ...
         } else if (this.puntos===8 || this.puntos===9) {
         ...
         this.strength.emit(this.puntos);
